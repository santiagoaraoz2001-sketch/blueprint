...
# Thread lock — protects _run_queues, _run_buffers, _event_counters
...
def publish_event(run_id: str, event_type: str, data: dict):
    ...
    Thread-safe — called from executor threads and the async event loop.
    ...
    with _lock:
        ...
        # Snapshot subscriber list for dispatch outside lock
        queues = list(_run_queues.get(run_id, []))

    # Push to live subscribers outside lock (put_nowait is atomic per-queue)
    for queue in queues:
        try:
            queue.put_nowait(event)
        except Exception:
            pass  # Dead or full subscriber
...
def _cleanup_run(run_id: str):
    ...
    Removes the event counter immediately, then defers buffer/queue removal
    by 30s so late-reconnecting clients can still replay the terminal event.
    """
    with _lock:
    ...
    async def event_generator():
        try:
            ...
            if lastEventId is not None:
                ...
                with _lock:
                    buffer = list(_run_buffers.get(run_id, []))

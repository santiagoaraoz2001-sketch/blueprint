...
export const BLOCKS_REFERENCE_TEXT = `Block Anatomy. Every block is a directory: blocks/{category}/{block_type}/ with block.yaml (metadata + config schema) and run.py (execution logic). block.yaml declares: name, type, category, inputs, outputs, config fields with types/defaults/bounds, timeout, max_retries. run.py exports def run(ctx: BlockContext). Block Categories: Data (25 blocks), Training (12 blocks), Inference (27 blocks), Evaluation (16+ blocks), Merge (5 blocks), Flow (22 blocks), Agents (9 blocks), Endpoints (14 blocks), Output (5 blocks). Special Blocks: A/B Significance Test compares two model variants with statistical significance testing (Welch's t-test, Mann-Whitney U, bootstrap). Reports p-value, Cohen's d, 95% CI, verdict. Model Diff loads two models, runs same prompt, compares top-K token probabilities, KL divergence, cosine similarity. BALLAST Training freezes most parameters, trains only embeddings + LayerNorm + output head (0.1-0.5% of params). Composite Blocks are sub-pipeline blocks for multi-agent orchestration. Block Validation checks inputs against block.yaml schema. Typed exceptions: BlockInputError, BlockConfigError, BlockTimeoutError, BlockMemoryError, BlockDependencyError, BlockDataError. Creating Custom Blocks: Option A LLM Block Generator (Cmd+G), Option B CLI scaffold_block.py, Option C external LLM prompt.`
